...
def displaySwimmers():
    ...
    return render_template("selectswimmer.html",
...
def displaySwimmerFiles():
    ...
    filelist = session["swimmers"][swimmerName]
    dictFileEvent = {}
    for file in filelist:
            swimmer, ageGroup, distance, stroke,*_ = swimclub.readSwimData(file)
            event = f"{swimmer} (Under {ageGroup} {distance} {stroke})"
            dictFileEvent[file] = event
            
    return render_template("selectswimmersevent.html",
